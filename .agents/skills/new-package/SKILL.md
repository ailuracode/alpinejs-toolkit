...
1. **Identify the category.** Confirm the classification in
   [.agents/instructions/architecture.instructions.md](../../instructions/architecture.instructions.md).
...
7. **Register the package** in the workspace + `toolkit.config.ts` (single
   source of truth — do NOT duplicate it in `AGENTS.md` or the root
   `README.md`).
