...
|---------|------|---------------|
| `@ailuracode/alpinejs-core` | Core | Plugin registry (`registerPlugin`, `initPlugins`) |
| `@ailuracode/alpinejs-theme` | Store | `$store.theme` |
| `@ailuracode/alpinejs-screen` | Store | `$store.device` |
| `@ailuracode/alpinejs-scroll` | Store | `$store.scroll` |
| `@ailuracode/alpinejs-sidebar` | Store | `$store.sidebar` |
| `@ailuracode/alpinejs-network` | Magic | `$network` |
| `@ailuracode/alpinejs-visibility` | Magic | `$visibility` |
| `@ailuracode/alpinejs-battery` | Magic | `$battery` |
| `@ailuracode/alpinejs-clipboard` | Magic | `$clipboard` |
| `@ailuracode/alpinejs-toast` | Magic | `$toast` — headless queue (no markup/CSS) |
| `@ailuracode/alpinejs-export` | Magic | `$export` |
| `@ailuracode/alpinejs-calendar` | Magic | `$calendar` |
| `@ailuracode/alpinejs-touch` | Magic | `$touch` |
| `@ailuracode/alpinejs-toggle` | Magic | `$toggle` |
| `@ailuracode/alpinejs-platform` | Magic | `$platform` |
| `@ailuracode/alpinejs-notify` | Magic | `$notify` |
| `@ailuracode/alpinejs-geo` | Store | `$store.geo` |
| `@ailuracode/alpinejs-share` | Magic | `$share` |
| `@ailuracode/alpinejs-attention` | Magic | `$wakelock`, `$idle` |
| `@ailuracode/alpinejs-query` | Core | Store-agnostic query cache (`createQueryClient`, `query({ adapter })`) |
| `@ailuracode/alpinejs-json-api` | Magic | `$jsonapi` — typed JSON:API client |
| `@ailuracode/alpinejs-query-adapter-nanostores` | Plugin | **Recommended** — Nanostores + `@nanostores/alpine` |
| `@ailuracode/alpinejs-query-adapter-alpine` | Plugin | Native `Alpine.reactive` adapter |
| `@ailuracode/alpinejs-query-adapter-zustand` | Plugin | Zustand vanilla adapter |
| `@ailuracode/alpinejs-query-devtools` | Plugin | Query cache devtools panel |
...
- **`@ailuracode/alpinejs-query`** — cache core only (`QueryCache`, `createQueryClient`, `query({ adapter })`, `vanillaQueryAdapter`)
- **`@ailuracode/alpinejs-query-adapter-nanostores`** — **recommended** for Alpine; uses Nanostores + [`@nanostores/alpine`](https://github.com/nanostores/alpine) (`x-nano`, `$nano`)
- **`@ailuracode/alpinejs-query-adapter-alpine`** — native `Alpine.reactive`, zero external store deps
- **`@ailuracode/alpinejs-query-adapter-zustand`** — Zustand vanilla stores; no official zustand-alpine exists — manual `subscribe` → `Alpine.reactive` bridge
...
- Query cache logic: use `createQueryClient()` with `vanillaQueryAdapter` (default) or an adapter from `@ailuracode/alpinejs-query-adapter-*`; use `startAlpine(query({ adapter: createAlpineNanostoresAdapter }))` for Alpine integration tests
...
2. `apps/demo/package.json` — add `"@ailuracode/alpinejs-<name>": "workspace:*"` to `dependencies`
