...
type MenuRegister = (alpine: AlpineBase) => void;

describe("menuPlugin — collision-avoidance keys", () => {
  it("registers under a custom storeKey", () => {
    const Alpine = createMockAlpine();
    (menuPlugin({ storeKey: "dropdown" }) as MenuRegister)(Alpine as unknown as AlpineBase);
  ...
  it("leaves the default storeKey untouched when no rename is supplied", () => {
    const Alpine = createMockAlpine();
    (menuPlugin({}) as MenuRegister)(Alpine as unknown as AlpineBase);
